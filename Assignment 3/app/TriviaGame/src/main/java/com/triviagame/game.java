...
public class game extends Activity implements OnClickListener {
	...
	int randomValue0to10 = 0;							//Random number between 1 and 30 for question ID's
	...
	int currentScore = 0;
	 ...
	 int RandomGenerator0To10(){
		 int randomInt2 = 0;
		 Random randomGenerator2 = new Random();
		 for (int idx2 = 1; idx2 <= 10; ++idx2) {
			 randomInt2 = randomGenerator2.nextInt(11);
		 }
		 return randomInt2;
	 }	 
	 ...
	 boolean questionsAlreadyDisplayed (int randomValue0to10){
		 for (int i = 0 ; i < 10 ; i++){
			 if (anArray2 [i] == randomValue0to10){
				 return true;	//question already displayed
			 }
			
		 }
		 anArray2 [numberOfQuestionsDisplayedCounter] = this.randomValue0to10; // questionId added to array of displayed questions
	...
	//Connects to the database to gather the question and 4 possible answers.  Uses DataBaseHelper class
	 void getInfoFromDatabase(int categoryId, int questionId){
		 Log.d(TAG, "connected to database and gathered the question");
			...
			try {
				Log.d(TAG, "display question from database");
				...
				Log.d(TAG, "display question from database end"); 
				myDbHelper.close();
	 ...
	 void increaseScore(){
		 Log.d(TAG, "increase the score by 10 points");
	...
	void decreaseScore(){
		Log.d(TAG, "decrease the score by 10 points");
		currentScore = currentScore - 10;
		com.triviagame.Trivia_Game.finalScore = currentScore;
	}
	...
	void gatherInfoBeforePopulating (){
		Log.d(TAG, "gathers question and answers from database");
		categoryId = 6;

	        	   //Loop until a valid questionId that hasn't been used is obtained
	        		while (validQuestionNumber == false){
	        			randomValue0to10 = RandomGenerator0To10();
	        			System.out.println("random11 in onClick = " + randomValue0to10);

	        		   alreadyDisplayQuestion = questionsAlreadyDisplayed (randomValue0to10);
	        	   ...
	        	   questionId = randomValue0to10;	//sets the valid random generated number to the questionID
	        ...
	        case R.id.nextquestion:
				Log.d(TAG, "case next question");
	        		if (nextOrFinishValue == 1){
		        		...
		        		startActivity(e);
						Log.d(TAG, "gathers question and answers from database if 1");
